...
- (void)addToolBarOption {
 ...
 //   CGFloat screenWidth = screenRect.size.width;
    ...
    //toolbar.frame = CGRectMake(0.0, 450.0, self.view.frame.size.width, 44);
    
   // toolbar.frame = CGRectMake(self.table.frame.origin.x, (self.table.frame.size.height - 44), self.table.frame.size.width, 44.0);
    
    toolbar.frame = CGRectMake(0, screenHeight - 104, self.view.frame.size.width, 44);
    NSLog(@"%f", screenHeight);
    NSLog(@"%f", self.table.frame.size.height);
...
- (void)viewDidLoad
{
    ...
    if (!self.isFromSearch && !self.isFromConcert) {
...
- (IBAction)shareButton
{
 ...
 //   NSString *artistName = artist.name;
  ...
  //  NSString *textToShare = [NSString stringWithFormat:@"%@ %@", @"Check out", artistName];
    ...
    NSString* artistURL3a = [NSString stringWithFormat: @"%@" , artist.url];
    
    NSString* artistURL3 = [artistURL3a substringFromIndex:16];
    
    NSString *artistURL2 = [NSString stringWithFormat: @"/%@/", [[NSUserDefaults standardUserDefaults] objectForKey:@"language"]];
    
    NSString *artistFinalURL = [NSString stringWithFormat: @"%@%@%@",artistURL1, artistURL2, artistURL3];
    NSString *artistFinalURLen = [NSString stringWithFormat: @"%@/en/%@",artistURL1, artistURL3];
    ...
    if (![[[NSUserDefaults standardUserDefaults] objectForKey:@"language"]  isEqual: @"es"])
        ...
        website = [NSURL URLWithString: artistFinalURL];
    
    
    else
        
        website = [NSURL URLWithString:artistFinalURLen];
    ...
    NSURL *imageUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.stagend.com/image/profile/%@/150/150", artist.artistId]];
