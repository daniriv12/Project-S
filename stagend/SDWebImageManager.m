...
#import "SDImageCache2.h"
#import "SDWebImageDownloader2.h"

static SDWebImageManager2 *instance;

@implementation SDWebImageManager2
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[SDWebImageManager2 alloc] init];
    }
...
{
    return [[SDImageCache2 sharedImageCache] imageFromKey:[url absoluteString]];
}
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority
{
    ...
    [[SDImageCache2 sharedImageCache] queryDiskCacheForKey:[url absoluteString] delegate:self userInfo:info];
}
...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegate>)delegate
{
    ...
    {
        SDWebImageDownloader2 *downloader = [[downloaders objectAtIndex:idx] retain];
...
- (void)imageCache:(SDImageCache2 *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    SDWebImageDownloader2 *downloader = [downloaderForURL objectForKey:url];

    if (!downloader)
    {
        downloader = [SDWebImageDownloader2 downloaderWithURL:url delegate:self userInfo:nil lowPriority:lowPriority];
...
- (void)imageDownloader:(SDWebImageDownloader2 *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        SDWebImageDownloader2 *aDownloader = [downloaders objectAtIndex:uidx];
    ...
    if (image)
    {
        ...
        [[SDImageCache2 sharedImageCache] storeImage:image
